...
function applyBrandPalette() {
    var cream = "#fff4de";
    var creamSoft = "#f7e7cc";
    var wineStroke = "#5b101c";
    var gold = "#d98e04";
    var goldSoft = "#f2c46a";
    var accent = "#730e0e";

    document.querySelectorAll(".mySVG [stroke='#3A5E77'], .mySVG [stroke='#3a5e77']").forEach(function (node) {
        node.setAttribute("stroke", wineStroke);
    });

    document.querySelectorAll(".mySVG [fill='#DDF1FA'], .mySVG [fill='#ddf1fa'], .mySVG [fill='#FFFFFF'], .mySVG [fill='#fff']").forEach(function (node) {
        node.setAttribute("fill", cream);
    });

    document.querySelectorAll(".mySVG [fill='#A9DDF3'], .mySVG [fill='#a9ddf3']").forEach(function (node) {
        node.setAttribute("fill", goldSoft);
    });

    if (mouthBG) mouthBG.setAttribute("fill", accent);
    if (mouthSmallBG) mouthSmallBG.setAttribute("fill", accent);
    if (mouthMediumBG) mouthMediumBG.setAttribute("fill", accent);
    if (mouthLargeBG) mouthLargeBG.setAttribute("fill", accent);
    if (tongue) tongue.querySelector("circle").setAttribute("fill", "#b33636");
    if (bodyBGchanged) bodyBGchanged.setAttribute("fill", creamSoft);

    var bgCircle = document.querySelector(".mySVG > circle");
    if (bgCircle) bgCircle.setAttribute("fill", goldSoft);

    var nosePath = document.querySelector(".nose");
    if (nosePath) nosePath.setAttribute("fill", wineStroke);

    var eyeDots = document.querySelectorAll(".eyeL circle:first-child, .eyeR circle:first-child");
    eyeDots.forEach(function (node) {
        node.setAttribute("fill", wineStroke);
    });

    var accentShapes = document.querySelectorAll(".twoFingers path[fill], .armL path[fill='#A9DDF3'], .armR path[fill='#a9ddf3']");
    accentShapes.forEach(function (node) {
        node.setAttribute("fill", gold);
    });
}
...
async function onLoginSubmit(event) {
    ...
    if (!passwordValue) {
        showFeedback("Escribe tu contraseña para continuar.", true);
        shakeLogin();
        password.focus();
        return;
    }
